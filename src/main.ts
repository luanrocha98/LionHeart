let display: Display = new Display("tela");
...
window.addEventListener('DOMContentLoaded', async function () {
  ...
  let corredorDormitorios: HTMLImageElement | null;
  let quartoIniv: HTMLImageElement | null;
  let cafeteria: HTMLImageElement | null = await ImageLoader.get(cafeteriaSrc);;

  dormitorio = await ImageLoader.get(dormitorioSrc);
  telhado = await ImageLoader.get(telhadoSrc);
  corredorDormitorios = await ImageLoader.get(corredorDormitoriosSrc);
  quartoIniv = await ImageLoader.get(quartoInivSrc);
...
let CenaD2: Dialog = new ScriptBuilder(1,
    new HeaderDialog({
...
let CenaC: Dialog = new ScriptBuilder(1,
    new HeaderDialog({
...
.addDialog(new DefaultDialog({
  ...
  .addDialog(new DefaultDialog({
    ...
    let CenaB1: Dialog = new ScriptBuilder(1,
    new HeaderDialog({
  ...
  let CenaB2: Dialog = new ScriptBuilder(1,
    new HeaderDialog({
        text: "Eu pensei em falar sobre a furadeira enquanto eu estava dormindo, mas o Plexu é bem aleatório assim as vezes,",
        speakers: [
          actorBuilder.setName("PensamentoLunado").build()
...
;
  ...
  let CenaB3: Dialog = new ScriptBuilder(1,
        new HeaderDialog({
          text: "Eu pensei em falar sobre a furadeira enquanto eu estava dormindo, mas o Plexu é bem aleatório assim as vezes,",
          speakers: [
            actorBuilder.setName("PensamentoLunado").build()
  ...
  let loader: GameLoader = new GameLoader(...Script.allActors);
  ...
  loader.setupObjects().then(() => {
    // CHAT AQUI
    let chat: DisplayChat = new DisplayChat("tela", scriptController.dialog.content);

    let dialogIsSurvey: boolean = false;
    let dialogIsBattle: boolean = false;
    let inBattle = false;
    setInterval(() => {
      window.requestAnimationFrame(async () => {
        if (inBattle) {
          let control = scriptController.dialog as Battle;
          control.tick();
        ...
        if (dialogIsSurvey) {
          let control = scriptController.dialog as Survey;
        ...
        scriptController.buildScene().build().controller((actors, background, listenners) => {
          ...
          if (dialogIsSurvey) {
            chat.drawSurvey(scriptController.dialog as Survey, title,fontColor);
          } else if (dialogIsBattle) {
            chat.drawBattle(scriptController.dialog as Battle, title);
          } else {
